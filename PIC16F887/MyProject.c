...
// --- Función personalizada de retardo variable ---
...
// --- Reproduce canción completa (genérica) ---
void PlaySong() {
    int dur;
    int i ;
    for ( i = 0; i < length; i++) {
...
// --- Melodía para selección de nivel / lobby ---
void PlayLobby() {
    int dur;
    int i;
    for ( i = 0; i < 20; i++) {
...
// --- Programa principal ---
void main() {
    ANSEL = 0;         // Desactivar entradas analógicas
    ANSELH = 0;
    ...
    while(1) {
        if (RB0_bit == 1) {      // Señal desde Arduino PC0 - Game Over
            PlayGameOver();
            while (RB0_bit);     // Espera hasta que se libere
        }

        if (RB1_bit == 1) {      // Señal desde Arduino PC1 - Reset
            PlayReset();
            while (RB1_bit);
        }

        if (RB2_bit == 1) {      // Señal desde Arduino PC2 - Lobby / Nivel
            PlayLobby();
            while (RB2_bit);
        }
    }
